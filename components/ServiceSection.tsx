...
const capIcons: Record<string, React.ReactNode> = {
  ai1: <LLMIcon />, ai2: <AutoIcon />, ai3: <ToolIcon />,
  web1: <SaaSIcon />, web2: <DesignIcon />, web3: <BackendIcon />,
}

/* ─── Tech stack ─────────────────────────────────────────── */
const TECH_STACK = ['Next.js', 'React', 'TypeScript', 'Node.js', 'Tailwind', 'PostgreSQL']
  ...
  const contentCol = (
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-8">
        <span className={`${accentText} text-xs font-mono tracking-widest`}>/ {phase}</span>
        <div className={`flex-1 h-px bg-gradient-to-r ${accentDivider} to-transparent`} />
      </div>

      {/* Title */}
      <h2 className="font-display text-3xl md:text-4xl lg:text-5xl font-extrabold text-white leading-tight mb-5
        drop-shadow-[0_2px_20px_rgba(0,0,0,0.8)]">
        {t(`${base}.title`) as string}
      </h2>

      {/* Description */}
      <p className="text-white/85 text-sm md:text-base leading-relaxed mb-8 max-w-md
        drop-shadow-[0_2px_16px_rgba(0,0,0,1)]">
        {t(`${base}.desc`) as string}
      </p>

      {/* Thin divider */}
      <div className={`h-px bg-gradient-to-r ${accentDivider} to-transparent mb-8`} />
      ...
      <div className="flex items-start gap-0">
        {stats.map((s, i) => (
          ...
          >
            <p className={`font-display text-2xl md:text-3xl font-extrabold ${accentText} leading-none mb-1
  ...
  const visualCol = (
    <div className="flex flex-col gap-3">
      ...
      {caps.map((cap, i) => (
        ...
        >
          <MovingBorderCard accent={isBlue ? 'blue' : 'amber'} duration={6 + i} innerClassName="p-5">
      ...
      {isAI ? (
        ...
        >
          <svg viewBox="0 0 240 60" className="w-full opacity-35" fill="none">
            {([[20,30],[70,10],[70,30],[70,50],[150,10],[150,30],[150,50],[220,30]] as [number,number][]).map(([x,y],i) => (
              <circle key={i} cx={x} cy={y} r="4" fill="#3b82f6" opacity=".8" />
            ))}
            {([[20,30,70,10],[20,30,70,30],[20,30,70,50],[70,10,150,10],[70,10,150,30],[70,30,150,30],[70,50,150,30],[70,50,150,50],[150,10,220,30],[150,30,220,30],[150,50,220,30]] as [number,number,number,number][]).map(([x1,y1,x2,y2],i) => (
              <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#3b82f6" strokeWidth=".8" opacity=".3" />
            ))}
          </svg>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-hidden
      >
        <span className={`font-display font-black text-[22rem] leading-none ${
